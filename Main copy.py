    ...
    for e in range(num):
        ...
        while attempts < max_attempts:
            ...
            if ((x - PlayerX.collider_jogador.x) ** 2 + (y - PlayerX.collider_jogador.y) ** 2) ** 0.5 >= min_distance_from_player:
                ...
                if not any(enemy_rect.colliderect(e.rect) for e in Enemies + new_enemies):
                    enemy_type = Classes.Enemy(x, y, 32, 32, WORLD_WIDTH, WORLD_HEIGHT, colliders, PlayerX, bullets, 
                                                2 if random.random() < 0.5 else 1, 
                                                pygame.image.load("Sprites/Walk.png"), 
                                                (255, 255, 0) if random.random() < 0.5 else (0, 255, 0))
